...
def choose_path(args):
    if 'assistant' not in vars(args).keys():
        teacher_str = args.teacher
    elif args.assistant == 0:
        teacher_str = 'nasty_' + args.teacher
    elif args.assistant == 1:
        teacher_str = 'reborn_' + args.teacher
    else:
        raise ValueError(r'No such assistant')
    if args.student == 'PLP' and args.ptype == 0:
        output_dir = Path.cwd().joinpath('outputs', args.dataset, teacher_str + '_' + args.student,
                                         'cascade_random_' + str(args.seed) + '_' + str(args.labelrate) + '_ind')
    elif args.student == 'PLP' and args.ptype == 1:
        output_dir = Path.cwd().joinpath('outputs', args.dataset, teacher_str + '_' + args.student,
                                         'cascade_random_' + str(args.seed) + '_' + str(args.labelrate) + '_tra')
    else:
        output_dir = Path.cwd().joinpath('outputs', args.dataset, teacher_str + '_' + args.student,
                                         'cascade_random_' + str(args.seed))
    check_writable(output_dir, overwrite=False)
    cascade_dir = Path.cwd().joinpath('outputs', args.dataset, teacher_str,
                                      'cascade_random_' + str(args.seed) + '_' + str(args.labelrate), 'cascade')
