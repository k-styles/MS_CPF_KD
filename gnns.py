...
class PLP(nn.Module):
    def __init__(self, g, num_layers, in_dim, emb_dim, num_classes, activation,
        ...
        self.masked_labels_one_hot = torch.mul(labels_one_hot, self.masked)
        self.alpha = nn.Parameter(torch.zeros(size=(self.node_num, 1)))
    ...
    def forward(self, features, label_init):
        ...
        for l in range(self.num_layers):
            ...
            middle_feats.append(h)
            h = torch.mul(h, ~self.masked) + self.masked_labels_one_hot
        # output projection
        logits = torch.mul(h, ~self.masked) + self.masked_labels_one_hot
        return logits, middle_feats, att, alpha, el, er
...
class PLPConv(nn.Module):
    def __init__(self,
        ...
        if self.ptype == 'ind':
            self.attn_l = nn.Parameter(torch.FloatTensor(size=(1, in_feats)))
        elif self.ptype == 'tra':
            self.fc_emb = nn.Parameter(torch.FloatTensor(size=(node_num, 1)))
        else:
    ...
    def forward(self, graph, feat, soft_label):
        with graph.local_scope():
            ...
            if self.ptype == 'ind':
                ...
                er = torch.zeros(graph.num_nodes(), device=graph.device)
            elif self.ptype == 'tra':
                feat_src = self.feat_drop(self.fc_emb)
                feat_dst = h_dst = torch.zeros(graph.num_nodes(), device=graph.device)
                el = feat_src
                er = feat_dst
            ...
            # graph.edata['e'] = torch.ones(graph.num_edges(), device=graph.device)  # non-parameterized PLP
            e = graph.edata.pop('e')
            # compute softmax
            ...
            graph.update_all(fn.u_mul_e('ft', 'a', 'm'),
                             fn.sum('m', 'ft'))
            if self.mlp_layers > 0:
                rst = torch.sigmoid(self.lr_alpha) * graph.dstdata['ft'] + \
                      torch.sigmoid(-self.lr_alpha) * self.mlp(feat)
            else:
            ...
            return rst, att, torch.sigmoid(self.lr_alpha).squeeze(), el.squeeze(), er.squeeze()
...
class DotGatConv(nn.Module):
    ...
    def forward(self, graph, feat, soft_label):
        ...
        # graph.edata['a'] = torch.ones(graph.num_edges(), device=graph.device)
        # Step 2. edge softmax to compute attention scores
        graph.edata['sa'] = edge_softmax(graph, graph.edata['a'])
        att = graph.edata['sa'].squeeze()
        cog_label = soft_label
        # cog_label = self.fc2(feat)
        # cog_label = torch.sigmoid(self.lr_alpha) * soft_label + torch.sigmoid(-self.lr_alpha) * self.fc2(feat)
        ...
        return rst, att, torch.sigmoid(self.lr_alpha).squeeze()
